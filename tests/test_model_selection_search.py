...
def test_grid_search_cv_list_of_dicts(binary_classification_data):
    ...
    param_grid = [{"C": [0.1, 1.0]}, {"C": [10.0, 100.0]}]
...
def test_randomized_search_cv_basic(binary_classification_data):
    ...
    search = RandomizedSearchCV(
        LogisticRegression(max_iter=100), param_dist, n_iter=3, cv=2, random_state=42
    )
    search.fit(X, y)
...
def test_randomized_search_cv_predict(binary_classification_data):
    ...
    search = RandomizedSearchCV(
        LogisticRegression(max_iter=100), param_dist, n_iter=2, cv=2, random_state=42
    )
    search.fit(X, y)
...
def test_randomized_search_cv_verbose(binary_classification_data, capsys):
    ...
    search = RandomizedSearchCV(
        LogisticRegression(max_iter=100), param_dist, n_iter=2, cv=2, verbose=1, random_state=42
    )
    search.fit(X, y)
...
def test_randomized_search_cv_regression(regression_data):
    ...
    search = RandomizedSearchCV(
        Ridge(), param_dist, n_iter=3, cv=KFold(n_splits=2), random_state=42
    )
    search.fit(X, y)
