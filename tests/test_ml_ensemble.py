...
def test_gradient_boosting_classifier_multiclass(classification_data):
    ...
    clf = GradientBoostingClassifier(
        n_estimators=10, learning_rate=0.1, max_depth=2, random_state=42
    )
    clf.fit(X, y)
...
def test_gradient_boosting_classifier_subsample(binary_classification_data):
    ...
    clf = GradientBoostingClassifier(n_estimators=10, subsample=0.8, random_state=42)
    clf.fit(X, y)
...
def test_gradient_boosting_regressor_subsample(regression_data):
    ...
    reg = GradientBoostingRegressor(n_estimators=10, subsample=0.7, random_state=42)
    reg.fit(X, y)
