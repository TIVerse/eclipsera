...
def test_mlp_classifier_activation_logistic():
    ...
    clf = MLPClassifier(
        hidden_layer_sizes=(10,), activation="logistic", max_iter=30, random_state=42
    )
    clf.fit(X, y)
...
def test_mlp_classifier_learning_rate():
    ...
    for lr in [0.001, 0.01, 0.1]:
        clf = MLPClassifier(
            hidden_layer_sizes=(10,), learning_rate_init=lr, max_iter=20, random_state=42
        )
        clf.fit(X, y)
        y_pred = clf.predict(X)
        assert y_pred.shape == y.shape
...
def test_mlp_classifier_alpha_regularization():
    ...
    for alpha in [0.0001, 0.001, 0.01]:
        clf = MLPClassifier(hidden_layer_sizes=(10,), alpha=alpha, max_iter=20, random_state=42)
        clf.fit(X, y)
        y_pred = clf.predict(X)
        assert y_pred.shape == y.shape
