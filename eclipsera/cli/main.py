...
def main() -> int:
    ...
    train_parser.add_argument("--data", type=str, required=True, help="Training data path (.npy or .csv)")
    train_parser.add_argument("--target", type=str, required=True, help="Target column name (for CSV) or target file (.npy)")
    train_parser.add_argument("--output", type=str, default="model.pkl", help="Output model path (default: model.pkl)")
    train_parser.add_argument("--task", type=str, choices=["classification", "regression"], default="classification", help="Task type")
    train_parser.add_argument("--cv", type=int, default=5, help="Cross-validation folds (default: 5)")
    
    # Predict command
    predict_parser = subparsers.add_parser("predict", help="Make predictions with a trained model")
    predict_parser.add_argument("--model", type=str, required=True, help="Trained model path (.pkl)")
    predict_parser.add_argument("--data", type=str, required=True, help="Input data path (.npy or .csv)")
    predict_parser.add_argument("--output", type=str, default="predictions.npy", help="Output predictions path (default: predictions.npy)")
    
    # Evaluate command
    evaluate_parser = subparsers.add_parser("evaluate", help="Evaluate a trained model")
    evaluate_parser.add_argument("--model", type=str, required=True, help="Trained model path (.pkl)")
    evaluate_parser.add_argument("--data", type=str, required=True, help="Test data path (.npy or .csv)")
    evaluate_parser.add_argument("--target", type=str, required=True, help="Target column name (for CSV) or target file (.npy)")
...
def _load_data(data_path: str, target: Optional[str] = None) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
    """Load data from file.
