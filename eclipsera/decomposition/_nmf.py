    ...
    def _initialize_nmf(self, X: np.ndarray, n_components: int) -> tuple:
        ...
        if self.init == "random":
            ...
            for j in range(1, min(n_components, len(S))):
                ...
                if np.linalg.norm(x_pos) * np.linalg.norm(y_pos) >= np.linalg.norm(
                    x_neg
                ) * np.linalg.norm(y_neg):
                    W[:, j] = (
                        np.sqrt(S[j] * np.linalg.norm(x_pos) ** 2 / np.linalg.norm(y_pos) ** 2)
                        * x_pos
                    )
                    H[j, :] = (
                        np.sqrt(S[j] * np.linalg.norm(y_pos) ** 2 / np.linalg.norm(x_pos) ** 2)
                        * y_pos
                    )
                else:
                    W[:, j] = (
                        np.sqrt(S[j] * np.linalg.norm(x_neg) ** 2 / np.linalg.norm(y_neg) ** 2)
                        * x_neg
                    )
                    H[j, :] = (
                        np.sqrt(S[j] * np.linalg.norm(y_neg) ** 2 / np.linalg.norm(x_neg) ** 2)
                        * y_neg
                    )
            ...
            if n_components > len(S):
                ...
                H[len(S) :, :] = (
                    np.abs(random_state.randn(n_components - len(S), n_features)) * 0.01
                )
