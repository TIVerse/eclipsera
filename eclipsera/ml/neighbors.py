...
class KNeighborsClassifier(BaseClassifier):
    ...
    def _compute_distances(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.metric == "euclidean":
            ...
            # Manhattan distance
            distances = np.sum(np.abs(X[:, np.newaxis, :] - self.X_[np.newaxis, :, :]), axis=2)
        elif self.metric == "minkowski":
            # Minkowski distance
            distances = np.sum(
                np.abs(X[:, np.newaxis, :] - self.X_[np.newaxis, :, :]) ** self.p, axis=2
            ) ** (1 / self.p)
        else:
...
class KNeighborsRegressor(BaseRegressor):
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.y_.ndim == 1:
            # Single output
            predictions = np.sum(neighbor_targets * weights, axis=1) / np.sum(weights, axis=1)
        else:
            # Multiple outputs
            weights_expanded = weights[:, :, np.newaxis]
            predictions = np.sum(neighbor_targets * weights_expanded, axis=1) / np.sum(
