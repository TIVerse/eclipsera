...
class MLPClassifier(BaseClassifier):
    ...
    def _backward_pass(
        self, X: np.ndarray, y: np.ndarray, activations: list, z_values: list
    ) -> tuple:
        ...
        for i in range(len(self.coefs_) - 1, -1, -1):
            coef_grads[i] = (
                activations[i].T @ delta / n_samples + self.alpha * self.coefs_[i] / n_samples
            )
        ...
        for iteration in range(self.max_iter):
            ...
            if self.solver == "sgd":
                ...
                for i in range(len(self.coefs_)):
                    ...
                    self.m_coefs_[i] = (
                        self.beta1 * self.m_coefs_[i] + (1 - self.beta1) * coef_grads[i]
                    )
                    self.m_intercepts_[i] = (
                        self.beta1 * self.m_intercepts_[i] + (1 - self.beta1) * intercept_grads[i]
                    )

                    # Update biased second raw moment estimate
                    self.v_coefs_[i] = self.beta2 * self.v_coefs_[i] + (1 - self.beta2) * (
                        coef_grads[i] ** 2
                    )
                    ...
                    # Update parameters
                    self.coefs_[i] -= (
                        learning_rate * m_hat_coef / (np.sqrt(v_hat_coef) + self.epsilon)
                    )
                    self.intercepts_[i] -= (
                        learning_rate * m_hat_intercept / (np.sqrt(v_hat_intercept) + self.epsilon)
                    )
